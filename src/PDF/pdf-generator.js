...
function pdfMoneyHtml(value) {
  return `<span dir="ltr">${pdfEsc(pdfMoney(value))}</span>`;
}
...
function pdfHotelRows(visit, labels) {
  ...
  if (hotel) rows.push(`<tr><td>${pdfEsc(labels.hotel)}</td><td>${pdfEsc(hotel.name)}</td><td>${hotel.nights}</td><td>${pdfMoneyHtml(hotel.nightlyPrice)}</td><td>${pdfMoneyHtml(hotel.total)}</td></tr>`);
  ...
  for (const service of [services.transfer, services.prosthesis, services.translator].filter(Boolean)) {
    rows.push(`<tr><td>${pdfEsc(pdfServiceLabel(service.name, labels))}</td><td>${service.included ? pdfEsc(labels.included) : '—'}</td><td>—</td><td>—</td><td>${service.total ? pdfMoneyHtml(service.total) : pdfEsc(labels.included)}</td></tr>`);
  }
...
function pdfVisitSummary(option, labels) {
  ...
  if (v1) rows.push(`<div class="visit-summary"><div class="visit-summary-title">${pdfEsc(labels.visit1)}</div><div class="visit-line"><span>${pdfEsc(labels.treatment)}</span><strong>${pdfMoneyHtml(v1.dentalTotal)}</strong></div><div class="visit-line"><span>${pdfEsc(labels.services)}</span><strong>${pdfMoneyHtml(v1.servicesTotal)}</strong></div><div class="visit-total"><span>${pdfEsc(labels.visit)} 1</span><strong>${pdfMoneyHtml(v1.total)}</strong></div></div>`);
  if (v2) rows.push(`<div class="visit-summary"><div class="visit-summary-title">${pdfEsc(labels.visit2)}</div><div class="visit-line"><span>${pdfEsc(labels.treatment)}</span><strong>${pdfMoneyHtml(v2.dentalTotal)}</strong></div><div class="visit-line"><span>${pdfEsc(labels.services)}</span><strong>${pdfMoneyHtml(v2.servicesTotal)}</strong></div><div class="visit-total"><span>${pdfEsc(labels.visit)} 2</span><strong>${pdfMoneyHtml(v2.total)}</strong></div></div>`);
  return rows.join('');
}
...
function pdfInstallmentBlock(quotation, option, labels) {
  ...
  return `<section class="installment-box"><div class="section-kicker">${pdfEsc(labels.installment)}</div><div class="installment-grid"><div><span>${pdfEsc(labels.package)} ${f.markupPercent}%</span><strong>${pdfMoneyHtml(financedPackage)}</strong></div><div><span>${pdfEsc(labels.installmentAmount)}</span><strong>${pdfMoneyHtml(installment)}</strong></div><div><span>${pdfEsc(labels.remainingCash)}</span><strong>${pdfMoneyHtml(remaining)}</strong></div><div><span>${pdfEsc(labels.cashPerVisit)}</span><strong>${pdfMoneyHtml(perVisit)}</strong></div></div></section>`;
}
...
<div class="patient-meta"><div class="patient-meta">
  ...
  <span> · </span>
  <span>
    <bdi dir="ltr">${quotation.options.length}</bdi>
