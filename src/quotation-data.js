...
function getQuotationHotelDetails(hotelId, roomType, nights){
  ...
  if(hotel.roomOptions){
    const room = hotel.roomOptions.find(item => {
      const name = item.name.toLowerCase();
      return name.includes(roomType.toLowerCase());
    });
...
function getQuotationProcedureDetails(card){
  return [...card.querySelectorAll('.procedure-choice:checked')]
    .map(choice => {
      const procedure = DUTY_PRICING.procedures.find(
  ...
  return {
    ...
    visits: {
      ...
      visit1: {
        ...
        hotel: getQuotationHotelDetails(
          visit1HotelId,
          visit1Room,
          visit1Nights
        ),
      ...
      visit2: visits === 2
        ? {
            ...
            services: {
              transfer: {
                name: 'VIP transfer',
                total: result.visit2Transfer
              },
              prosthesis: {
                name: 'Dental prosthesis',
                total: result.visit2Prosthesis
              },
              translator: {
                name: 'Translator',
                total: 0,
...
if(typeof window !== 'undefined'){
  window.buildQuotationData = buildQuotationData;
}
